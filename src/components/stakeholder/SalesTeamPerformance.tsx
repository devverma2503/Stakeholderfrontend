          ...
          <div>
            ...
            <div className="sm:hidden space-y-3">
              {sortedData.map((agent) => (
                <div key={agent.id} className="bg-white rounded-lg shadow p-3 border">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-600">{agent.zone} • Since {agent.joinDate}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-gray-900">{formatCurrency(agent.revenue)}</div>
                      <div className="text-xs text-gray-500">{agent.collegesClosed}/{agent.collegesTotalAllocated} colleges</div>
                    </div>
                  </div>

                  <div className="mt-3 flex items-center justify-between">
                    <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getTargetColor(agent.targetAchievement)}`}>{agent.targetAchievement}%</div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-center py-3 px-4 font-semibold text-gray-900">Target Achievement</th>
                  ...
                  {sortedData.map((agent) => (
                    <tr key={agent.id} className="hover:bg-gray-50 transition-colors">
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-gradient-to-br from-emerald-500 to-blue-500 rounded-full flex items-center justify-center text-white font-bold text-xs">{agent.avatar}</div>
                          <div>
                            <div className="font-medium text-gray-900 text-sm">{agent.name}</div>
                      ...
                      <td className="py-4 px-4"><span className="text-sm text-gray-700">{agent.zone}</span></td>
                      <td className="py-4 px-4 text-right">
                        <div className="text-sm font-semibold text-gray-900">{agent.collegesClosed}</div>
                        <div className="text-xs text-gray-600">of {agent.collegesTotalAllocated}</div>
                      </td>
                      <td className="py-4 px-4 text-right"><span className="font-semibold text-gray-900">{formatCurrency(agent.revenue)}</span></td>
                      <td className="py-4 px-4 text-center"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getTargetColor(agent.targetAchievement)}`}>{agent.targetAchievement}%</span></td>
                      <td className="py-4 px-4 text-center"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(agent.status)}`}>{agent.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
