...
const RevenueSummaryCards = ({
    ...
    const getFilteredData = () => {
        ...
        if (filteredProducts.length > 0) {
            if (filteredProducts[0].toLowerCase().includes("printed notes"))
                groupMultiplier = 0.7;
        ...
        const topPerformingSubjectValue =
            selectedSubject !== "all"
                ...
                ? MEDICAL_SUBJECTS[
                      Math.floor(Math.random() * MEDICAL_SUBJECTS.length)
                  ]
                : "DigiOne";
        ...
        if (selectedSubject !== "all") {
            ...
            baseMultiplier *=
                subjectIndex !== -1 ? (20 - subjectIndex) / 20 : 0.5;
        }
        ...
        const currentSubscriptions = Math.floor(
            baseSubscriptions * (0.9 + Math.random() * 0.2)
        );
        const currentAvgRevenue =
            (baseRevenue / baseColleges) * (0.9 + Math.random() * 0.2);
        const currentSubjectRevenue =
            baseRevenue * 0.3 * (0.8 + Math.random() * 0.4);
        ...
        const previousSubscriptions = Math.floor(
            currentSubscriptions / (1 + subscriptionGrowth / 100)
        );
        const previousAvgRevenue = currentAvgRevenue / (1 + avgGrowth / 100);
        const previousSubjectRevenue =
            currentSubjectRevenue / (1 + subjectGrowth / 100);
    ...
    const cards = [
        {
            ...
            previousPercent: makePercent(
                data.previousRevenueNum,
                data.totalRevenueNum
            ),
        ...
        },
        {
            ...
            value: `${
                selectedZone !== "all"
                    ? selectedZone.endsWith(" Zone")
                        ? selectedZone.replace(" Zone", "")
                        : selectedZone
                    : "North"
            } (${data.avgRevenuePerCollege})`,
            previousValue: data.previousAvgRevenue,
            previousPercent: makePercent(
                data.previousAvgRevenueNum,
                data.avgRevenuePerCollegeNum
            ),
        ...
        },
        {
            ...
            previousPercent: makePercent(
                data.previousSubjectRevenueNum,
                data.subjectRevenueNum
            ),
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            {cards.map((card, index) => (
                <div
                    key={index}
                    className={`${card.bgColor} rounded-xl p-6 border border-gray-200/50`}
                >
                        ...
                        <div className=" border-t border-gray-200/30">
                            <div className="flex items-center justify-between text-xs">
                                <div className="text-gray-500">
                                    <span
                                        className={`ml-0 text-xs font-medium ${card.changeColor} bg-white/70 px-2 py-1 rounded-full flex items-center gap-1`}
                                    >
                                        {card.previousValue}
                                        <span className="text-xs text-gray-500 ml-1">
                                            Previous Year
