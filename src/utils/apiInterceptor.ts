// API Interceptor - auth redirect disabled to avoid forcing login on dashboard access
    ...
    try {
      ...
      if (response.status === 401 && this.maxRetries > 0) {
        ...
        if (newToken) {
          ...
          // Refresh failed — do not force navigation to login. Log and surface error instead.
          // useAuthStore.getState().logout();

          // Redirect to login page (disabled)
          // if (typeof window !== 'undefined') {
          //   window.location.href = '/auth/login';
          // }

          console.warn('Authentication failed during API call. Login redirect suppressed.');
